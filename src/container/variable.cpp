...
Variable::Variable(float data, bool requires_grad) {
	impl = std::make_shared<VariableImpl<float>>(data, requires_grad);
}

Variable::Variable(std::shared_ptr<VariableImpl<float>> impl) : impl(impl) {}
...
void Variable::backward() {
    ...
    while (!funcs.empty()) {
		...
		std::shared_ptr<VariableImpl<float>> input = f->get_input();
		std::shared_ptr<VariableImpl<float>> output = f->get_output();
